...
# Dette scriptet henter Microsoft Core fonts som Microsoft tidligere har
# distribuert ut for å gjøre dem tilgjengelige for alle på nett, fra
# Core Fonts-prosjektet på sourceforge
...
# for hver fil, last ned, sjekk sha256-sum, hent ut ttf og rydd opp
for i in {0..10}; do
  wget ${source[i]};
  echo ${sha256sums[i]} ${_files[i]} | sha256sum -c;
  cabextract -F "*.TTF" ${_files[i]} -d ../ttf/;
  rm ${_files[i]};
done;
