...
import static no.nav.dokarkivavlevering.avlevering.repository.SqlQueries.FINN_START_SLUTT_DATO_SQL;
...
public class AvleveringRepository {
	...
	public AvleveringRepository(NamedParameterJdbcTemplate namedParameterJdbcTemplate) {
	...
	public StartSluttDato findStartOgSluttdato(String tema) {
		return namedParameterJdbcTemplate.query(FINN_START_SLUTT_DATO_SQL, Map.of("tema", tema), START_SLUTT_DATO_RESULTSET_EXTRACTOR)
				.stream().findFirst().orElseThrow();
	}
	...
	private static final ResultSetExtractor<List<StartSluttDato>> START_SLUTT_DATO_RESULTSET_EXTRACTOR = JdbcTemplateMapperFactory.newInstance()
			.newResultSetExtractor(StartSluttDato.class);
