package no.nav.dokarkivavlevering.core.azure;
...
public class AzureOAuthEnabledWebClientConfig {
	...
	@Bean
	List<ClientRegistration> clientRegistration(AzureProperties azureProperties, DokarkivavleveringProperties dokarkivavleveringProperties) {
		return List.of(
				ClientRegistration.withRegistrationId(CLIENT_REGISTRATION_PDL)
						...
						.scope(dokarkivavleveringProperties.getEndpoints().getPdl().getScope())
						.build()
		);
	}
}
